...
// relevance.
$first_name 	= ($_GET['first_name'] == '') ? '' : " `first_name` = '" . DB::clean($_GET['first_name']) . "' AND ";
$last_name 		= ($_GET['last_name'] == '') ? '' : " `last_name` = '" . DB::clean($_GET['last_name']) . "' AND ";

$year 			= ($_GET['year'] == '0') ? '' : " `year` = '" . DB::clean($_GET['year']) . "' AND ";
$major 			= ($_GET['major'] == '0') ? '' : " `major` = '" . DB::clean($_GET['major']) . "' AND ";
$dorm 			= ($_GET['dorm'] == '0') ? '' : " `dorm` = '" . DB::clean($_GET['dorm']) . "' AND ";
$recruit_place 	= ($_GET['recruit_place'] == '0') ? '' : " `recruit_place` = '" . DB::clean($_GET['recruit_place']) . "' AND ";
$texting 		= ($_GET['texting'] == '-1') ? '' : " `texting` = '" . DB::clean($_GET['texting']) . "' AND ";
$hometown 		= ($_GET['hometown'] == '') ? '' : " `hometown` = '" . DB::clean($_GET['hometown']) . "' AND ";
$address 		= ($_GET['address'] == '') ? '' : " `address` = '" . DB::clean($_GET['address']) . "' AND ";
$phone 			= ($_GET['phone'] == '') ? '' : " `phone` = '" . DB::clean($_GET['phone']) . "' AND ";
$email 			= ($_GET['email'] == '') ? '' : " `email` = '" . DB::clean($_GET['email']) . "' AND ";
...
foreach ($tags_bad as $item) {
	$tags .= $tags_table . "LIKE '%" . DB::clean($item) . "%' AND ";
}

foreach ($positions_bad as $item) {
	$positions .= $positions_table . "LIKE '%" . DB::clean($item) . "%' AND ";
}
...
if ($first_name !== '' || $last_name !== '') {
	...
	$possible_results = DB::search("SELECT `id` FROM `{$database}`.`users` WHERE " . substr($first_name . $last_name, 0, -5));
} else {
	...
	$sql2 .= $tags . $positions;
	$possible_results = DB::search($sql2);
}
...
foreach ($possible_results as $user) {
	...
	$tmp_data = DB::single("SELECT `first_name`,`last_name` FROM `{$database}`.`users` WHERE `id` = '{$user['id']}' LIMIT 1");
	$users[$user['id']] = array(
